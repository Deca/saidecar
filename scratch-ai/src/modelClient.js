...
export async function askModel({ question, modeName }) {
  const mode = modes[modeName] || modes.normal;

  if (config.backend === "codex" && mode.tools.length > 0) {
    return askCodexWebSearch({ question, modeName });
  }

  if (config.backend === "codex") {
    return askCodex({ question, modeName });
  }

  return askOpenAI({ question, modeName });
}
